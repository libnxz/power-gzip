		...
		union {
			...
			struct {
				...
				union {
					 ...
					 * part of the resume function, along
					 * with the adjusted source.
					 * Note that the deflate standard has
					 * reversed bit ordering within each
					 ...
					 * stream will typically effect the SUBC
					 * count.
					 * For ZLIB and GZIP these values are 32
					 * and 64 bits respectively. Since the
					 * trailer is byte aligned up to 7 unused
					 * bits may be present between the
					 * Deflate block and the trailer, and
					 * therefore the values may range from
					 * 32 to 39, and 64 to 71 bits
					 * respectively.
					 */
					uint32_t out_subc;
				};

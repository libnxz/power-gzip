			       ...
			       nx_gzip_crb_cpb_t *cmdp, nx_devp_t handle)
{
	...
	/* resuming with no history;
	   not optimal but good enough for the sample code */
	...
	initial_pass_done:
		...
		if (initial_pass == 1) {
			...
			srclen = NX_MIN(NX_MAX(((uint64_t)srclen *
				   NX_LZ_SAMPLE_PERCENT)/100, NX_LZ_SAMPLE_NBYTE),
				   srclen);
		...
		/* Fault-in pages; an improved code wouldn't touch so
		   ...
		   touch amounts */
		_nx_touch_pages (cmdp, sizeof(*cmdp), pagelen, 0);
		...
		if (cc == ERR_NX_AT_FAULT) {
			...
			if (fault_tries > 0) {
				...
				fprintf(stderr, "error: cannot progress; too many"
					" faults\n");
				exit(-1);
			};
		}

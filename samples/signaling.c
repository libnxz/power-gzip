...
static void install_new_handler(void *handler, struct sigaction *old,
				const char *comment)
{
	int rc;
	...
	fprintf(stderr, "installed handler %p for %s and retrieved old handler "
...
{
	fprintf(stderr, "%s(pid=%d): signal %d si_code %d, si_addr %p, info %p,"
		" ctx %p\n", __FUNCTION__, getpid(), sig, info->si_code,
		info->si_addr, (void *) info, (void *)ctx);
	fflush(stderr);

	if (make_believe_jvm_saved_this_old_handler.sa_sigaction != NULL) {
		fprintf(stderr, "%s: calling the downstream handler %p\n",
			__FUNCTION__,
			make_believe_jvm_saved_this_old_handler.sa_sigaction);
		make_believe_jvm_saved_this_old_handler.sa_sigaction(sig, info,
								     ctx);
	}
...
void our_library_sigsegv_handler(int sig, siginfo_t *info, void *ctx)
{
	...
	if ( sig == SIGSEGV && ((random() % 2) == 0) ) {
		...
		fprintf(stderr, "%s: ++++ SIGNAL FOR US. We touch info->si_addr"
			" %p and return. Not calling downstream handler\n\n",
		...
		fprintf(stderr, "%s: ---- SIGNAL *NOT* FOR US. Calling any "
			"found downstream handler %p\n", __FUNCTION__,
			our_library_saved_this_old_handler.sa_sigaction);
	}
...
int main1()
{
	...
	install_new_handler(make_believe_jvm_handler,
			    &make_believe_jvm_saved_this_old_handler, "JVM");
	/* our_library comes later and inserts itself at the head of chain */
	install_new_handler(our_library_sigsegv_handler,
			    &our_library_saved_this_old_handler, "OUR_LIBRARY");
#else
	...
	fprintf(stderr, "our process id is %d : from another bash shell do "
		"several times: kill -SIGSEGV %d\n", getpid(), getpid());
...
int main2()
{
	...
	install_new_handler(our_library_sigsegv_handler,
			    &our_library_saved_this_old_handler, "OUR_LIBRARY");
	fprintf(stderr, "our process id is %d : from another bash shell do "
		"several times: kill -SIGSEGV %d, after 5 kills old handler "
		"will be restored\n", getpid(), getpid());
